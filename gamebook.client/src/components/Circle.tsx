...
const Circle: React.FC<CircleProps> = ({ npc, content }) => {
    ...
    const fast = 600;
    const slow = 1700;
    const duration = equipedWeapon
        ? (100 - equipedWeapon.damage) * 14 >= fast
            ? (100 - equipedWeapon.damage) * 14
            : fast
        : slow;
        ...
        const handleKeyPress = (event: KeyboardEvent) => {
            if (event.code === "Space") {
                ...
                if (currentAngle >= emptyStart && currentAngle < emptyEnd) {
                    ...
                    setNpcsHp(
                        (prevNpcsHp: number) =>
                            prevNpcsHp - (equipedWeapon?.damage || 1)
                    );
                    setAnimationClass(styles.hit);
                    setTimeout(() => {
                        setAnimationClass("");
                    }, 500);
                } else if (
                    currentAngle >= emptyEnd &&
                    currentAngle < filledEnd
                ) {
                    //critical hit
                    setNpcsHp(
                        (prevNpcsHp: number) =>
                            prevNpcsHp - (equipedWeapon?.damage || 1) * 2
                    );
                    setAnimationClass(styles.criticalhit);
                    setTimeout(() => {
                        setAnimationClass("");
                    }, 500);
                } else {
    ...
    useEffect(() => {
        ...
        if (npcsHp <= 0) {
            ...
            setInteractiblesRemovedFromLocation(
                (prevInteractiblesRemovedFromLocation: LocationContent[]) => {
    ...
    return (
        <div className={styles.circle}>
            <div className={`${styles.npc} ${animationClass}`}>
                <img
                    style={{
                        WebkitFilter: `drop-shadow(4px 4px 2px rgba(255, 0, 0, ${
                            1 - npcsHp / npc.health
                        })) drop-shadow(-4px -4px 2px rgba(255, 0, 0, ${
                            1 - npcsHp / npc.health
                        }))`,
                        filter: `drop-shadow(4px 4px 2px rgba(255, 0, 0, ${
                            1 - npcsHp / npc.health
                        })) drop-shadow(-4px -4px 2px rgba(255, 0, 0, ${
                            1 - npcsHp / npc.health
                        }))`,
